...
export default function Puestos() {
  ...
  const cargarPuestosDesdeRelacionCampos = async () => {
    try {
      ...
      // 1. Intentamos leer la tabla de configuración probando variaciones en el nombre
      ...
      // 2. Extracción directa desde la tabla empleados para asegurar el respaldo sincronizado
      ...
      // Eliminamos puestos duplicados basados en el nombre
  ...
  const eliminarPuestoDefinitivo = async (puesto) => {
    ...
    const confirmar = window.confirm(
      `¿Estás COMPLETAMENTE seguro de eliminar el puesto "${puesto.nombre}" de forma definitiva?`
    );
    if (!confirmar) return;
